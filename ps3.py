...
import random
import os
os.chdir('C:\\Users\\Duc Pham\\temp\\MIT\\MIT OCW intro course\\6.0002')
...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        check = [width, height, dirt_amount]
        for param in check:
            if not isinstance(param, int):
                raise TypeError("{} is not an integer.".format(param))
            elif param <= 0:
                raise ValueError("{} is not positive.".format(param))
            else:
                pass
        ...
        # self.tiles: a dictionary with keys representing tiles in tuples and values representing current dirt amount
        # in the tiles
        self.tiles = {}
        width_div = [m for m in range(0, width)]
        height_div = [n for n in range(0, height)]
        keys = []
        for m in width_div:
            for n in height_div:
                keys.append((m, n))
        for key in keys:
            self.tiles[key] = self.dirt_amount
    ...
    def get_num_cleaned_tiles(self):
        ...
        return len([dirt for dirt in self.tiles.values() if dirt == 0])
        # return len([(m, n) for (m, n) in self.tiles.keys() if self.is_tile_cleaned(m, n)])
    ...
    def is_position_in_room(self, pos):
        ...
        if isinstance(pos, Position):
            if pos.get_x() < 0 or pos.get_x() > self.width or pos.get_y() < 0 or pos.get_y() > self.height:
                return False
            else:
                return True
        else:
            raise TypeError("{} is not a Position object!".format(pos))
...
class Robot(object):
    ...
    def __init__(self, room, speed, capacity):
        ...
        if isinstance(room, RectangularRoom):
            ...
            raise TypeError("Input room must be a RectangularRoom object!")
        ...
        try:
            ...
            if speed > 0:
                ...
                raise ValueError("Input speed is not positive!")
        except ValueError:
            print("Input speed must be a float!")

        if isinstance(capacity, int):
            if capacity > 0:
                self.capacity = capacity
            else:
                raise ValueError("Input cleaning capacity is not positive!")
        else:
            raise TypeError("Input cleaning capacity must be an integer!")
    ...
    def set_robot_position(self, position):
        ...
        if isinstance(position, Position):
            ...
            raise TypeError("Input position must be a Position object!")
    ...
    def set_robot_direction(self, direction):
        ...
        if isinstance(direction, float):
            if 0 <= direction < 360:
                ...
                raise ValueError("Input direction must be non-negative and smaller than 360!")
        else:
            raise TypeError("Input direction must be a float!")
...
class FurnishedRoom(RectangularRoom):
    ...
    def get_num_tiles(self):
        ...
        return len([tile for tile in self.tiles if tile not in self.furniture_tiles])
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        if self.room.is_position_valid(new_pos):
            ...
            # print("Next position invalid, rotating...")
            while True:
                new_direction = random.uniform(0, 360)
                if new_direction in [360, self.direction]:
                    continue
                else:
                    self.set_robot_direction(new_direction)
                    # print("Facing towards {} degree.".format(self.direction))
                    break
...
class FaultyRobot(Robot):
    ...
    def set_faulty_probability(prob):
        ...
        """
        FaultyRobot.p = prob # why no self.p
    ...
    def update_position_and_clean(self):
        ...
        fault_state = self.gets_faulty()
        # print("Faulty?", fault_state)
        if fault_state:
            while True:
                new_direction = random.uniform(0, 360)
                if new_direction not in [360, self.direction]:
                    self.set_robot_direction(new_direction)
                    break
                else:
                    continue
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    while trial < num_trials:
        trial += 1
        # run trial; copy some codes from ps3_verify_movement27.py
        time_steps = 0
        coverage = 0
        ...
        print("Number of time steps for Trial {}:".format(trial), time_steps)
        numstep_list.append(time_steps)

    return sum(numstep_list) / len(numstep_list)

# run_simulation() argument names:
...
#       of a 20x20 room?
#
# avg_step_std = run_simulation(1, 1.0, 1, 20, 20, 3, 0.8, 50, StandardRobot)
# avg_step_faulty = run_simulation(1, 1.0, 1, 20, 20, 3, 0.8, 50, FaultyRobot)
# print ('avg time steps: ' + str(avg_step_std))
# print ('avg time steps: ' + str(avg_step_faulty))
# print ('standard robot is {} times more efficient than faulty robot'.format(round(avg_step_faulty / avg_step_std, 2)))
#
...
import sys
# # import os # already import for this file
#
@contextmanager
# suppress print out temporarily
def suppress_stdout():
...
# for width, height in [(10, 30), (20, 15), (25, 12), (50, 6)]:
#     num_trials = 50
#     with suppress_stdout():
#         avg_step_std = run_simulation(1, 1.0, 1, width, height, 3, 0.8, num_trials, StandardRobot)
#         avg_step_faulty = run_simulation(1, 1.0, 1, width, height, 3, 0.8, num_trials, FaultyRobot)
#     # cprint('For room dimension {} x {}:'.format(width, height), 'yellow')
#     print("----- ROOM DIMENSION {} x {} -----".format(width, height))
#     print("running {} trials for each robot type...".format(num_trials))
#     print('avg time steps for StandardRobot: ' + str(avg_step_std))
#     print('avg time steps for FaultyRobot: ' + str(avg_step_faulty))
#     print('standard robot is {} times more efficient than faulty robot'
#           .format(round(avg_step_faulty / avg_step_std, 2)))
